...
SELECT COUNT(*) AS jumlah_data FROM amz;

SELECT Price FROM amz;

#Cek tipe data
DESC amz;
#Ubah data """" menjadi NULL
UPDATE amz SET Cust_id = NULL WHERE Cust_id = '""""';
#Ubah koma jadi titik
UPDATE amz SET Price = REPLACE(Price, ',', '.');
#Ubah tipe data
ALTER TABLE amz MODIFY COLUMN Price FLOAT;

SELECT Amerika FROM gms;
